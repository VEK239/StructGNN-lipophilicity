...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        elif self.dataset == 'logp/train' or self.dataset == 'logp/val' or self.dataset == 'logp/test':
            smiles_list, rdkit_mol_objs, labels = \
                _load_logp_dataset(self.raw_paths[0])
            for i in range(len(smiles_list)):
                print(i)
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
...
def _load_logp_dataset(input_path):
    """
